import { Box, Card, CardActionArea, Avatar, CardHeader, CardMedia, Typography, makeStyles, CardContent, LinearProgress } from '@material-ui/core'
...
const { getMySavedAlbums, setAccessToken, getArtists, getMyRecentlyPlayedTracks, getMyTopTracks, getMyCurrentPlaybackState } = new Spotify()
...
export const AnalyticPage = () => {
  ...
  const [currentPlayback, setCurrentPlayback] = useState<SpotifyApi.CurrentPlaybackResponse | null>(null)
  ...
  useEffect(() => {
    if (token) {
      ...
      Promise.all([
        ...
        .then(([res1, res2, res3]) => getArtists(
          uniq([
        ...
        .then(res => setArtistsInfo(res.artists))
    }
  }, [token])
  ...
  return (
    <div>
      {currentPlayback && (
        <Box>
          <Typography variant="h2" gutterBottom>Now Playing</Typography>
              ...
              <Box width="300px">
                <CardContent style={{ height: '100%' }}>
                  <Box display="flex" flexDirection="column" height="100%">
                    ...
                    {currentPlayback.progress_ms && currentPlayback.item?.duration_ms && (
                      <LinearProgress variant="determinate" value={currentPlayback.progress_ms / currentPlayback.item?.duration_ms * 100} />
                    )}
                  </Box>
                </CardContent>
              </Box>
      ...
      <Box display="flex" flexWrap="wrap">
        {latestAlbums.map(({ album: { images, name, artists, uri } }) => {
          const artistInfo = artistsInfo.find(info => info.id === artists[0]?.id)
          return (
            <Card key={name} className={classes.card}>
              <CardHeader
                avatar={
                  artistInfo && (
                    <CardActionArea onClick={() => window.location.assign(artistInfo.uri)}>
                      <Avatar variant="rounded" src={artistInfo?.images[0]?.url} className={classes.avatar} />
                    </CardActionArea>
                  )
                }
                title={name}
      ...
      <Typography variant="h2" style={{ marginTop: 50 }}>Top Tracks</Typography>
      <Box display="flex" flexWrap="wrap">
      ...
      <Typography variant="h2" style={{ marginTop: 50 }}>Latest Songs</Typography>
      <Box display="flex" flexWrap="wrap">
        {recentlyPlayedTracks.map(({ track: { album: { images }, name, artists, uri }, played_at }) => {
          const artistInfo = artistsInfo.find(info => info.id === artists[0]?.id)
          return (
